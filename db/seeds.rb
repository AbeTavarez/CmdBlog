...
@pythonHack = Article.create!({title: "HackingWithPython", description: "This live map in the norsecorp website gives you real-time information of which countries are currently being cyber-attacked! This is just one of those avenues to understand how the importance of enhancing cybersecurity in everything we build is the need of the hour — rather, need of the minute!
  ...
  In this lesson, we will learn about why Python is one of the most in-demand skills among cybersecurity recruits and also try to learn and understand the fundamentals of the language.",image_path: "https://i.morioh.com/a961469f8b.png", topic: "coding", user_id: @user.id, category_id: @hacking.id})
...
@swiftdev = Article.create!({title: "App Dev With Swift", description: "A cheat sheet can be a handy tool to quickly reference Xcode, Swift or iOS logic from a compact overview. Ideally, you can print them out and lay them down on your desk. This makes it really easy to access and use them often. Therefore, I’ll show you a few useful cheat sheets which you can use during development.
  ...
  @pythonNew = Article.create!({title:"New Features in Python 3.9", description: "This one is more of an out-of-sight change but has the potential of being one of the most significant changes for the future evolution of Python.
    ...
    If you’re wondering if all this is worth the investment, just consider the developer’s salaries. Even 10% more productivity is a LOT!",image_path: "https://hackernoon.com/drafts/6t8232hs.png", topic: "productivity", user_id:@user.id, category_id: @programming.id})
