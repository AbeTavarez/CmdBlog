...
@swiftdev = Article.create!({title: "App Dev With Swift", description: "A cheat sheet can be a handy tool to quickly reference Xcode, Swift or iOS logic from a compact overview. Ideally, you can print them out and lay them down on your desk. This makes it really easy to access and use them often. Therefore, I’ll show you a few useful cheat sheets which you can use during development.
  ...
  @pythonNew = Article.create!({title:"New Features in Python 3.9", description: "This one is more of an out-of-sight change but has the potential of being one of the most significant changes for the future evolution of Python.
    ...
    LL(1) creates limitations in the Python syntax (without possible workarounds). This issue highlights that the following code simply cannot be implemented using the current parser (raising a SyntaxError):", topic: "Python Programming", user_id: @user0.id, category_id: @programming.id})

    @appideas = Article.create!({title: "15 App Ideas to Build and Level Up your Coding Skills", description: "We all know that it can be hard sometimes to find new application ideas that you could build in order to either improve or learn a new programming language or framework. We divided these app ideas into three tiers based on the knowledge and experience required to complete them. The tiers are: Beginner, Intermediate and Advanced.", topic:"Programming", user_id: @user0.id ,category_id: @programming.id})
    ...
    If you’re wondering if all this is worth the investment, just consider the developer’s salaries. Even 10% more productivity is a LOT!", topic: "productivity", user_id:@user.id, category_id: @programming.id})
